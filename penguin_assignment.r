...
## set working directory, this will be different when working in your own device 
setwd("~/PenguinProject")

##load the data, the directory will be different depending on where the palmer penguins data is stored in your laptop 
...
plot_Gentoo_figure <- function(Gentoo_sex_mass) {  
  Gentoo_sex_mass %>%
    ...
    labs(title = "Distribution of female and male mass in Gentoo penguins", 
         x = "Body mass (g)", y = "Number of penguins") + theme_classic() + 
...
save_Gentoo_plot_png(Gentoo_histogram, "figures/Gentoo_sex_mass_report.png", 
                      size = 15, res = 600, scaling = 1)

## Save the plot for a presentation
save_Gentoo_plot_png(Gentoo_histogram, "figures/Gentoo_sex_mass_powerpoint_2.png", 
                      size = 15, res = 600, scaling = 1.1)

## Save the plot for a poster
save_Gentoo_plot_png(Gentoo_histogram, "figures/Gentoo_sex_mass_poster.png", 
                      size = 30, res = 600, scaling = 2.2)
...
save_Gentoo_plot_svg(Gentoo_histogram, "figures/Gentoo_sex_mass.svg", 
                      size = 15, scaling = 1)
